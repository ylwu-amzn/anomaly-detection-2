...
public class ADBatchTaskRunner {
    ...
    private ActionListener<ADBatchAnomalyResultResponse> getInternalHCDelegatedListener() {
        return ActionListener
            .wrap(
                r -> { logger.debug("[InternalHCDelegatedListener]: running task on node {}", r.getNodeId()); },
    ...
    private float calculateInitProgress(String taskId) {
        ...
        return rcf.getTotalUpdates() >= NUM_MIN_SAMPLES ? 1.0f : (float) rcf.getTotalUpdates() / NUM_MIN_SAMPLES;
    }
